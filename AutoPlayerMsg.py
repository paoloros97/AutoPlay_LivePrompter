...
try:
    ...
    inports = [mido.open_input(inputMIDIport), mido.open_input(inputMsgMIDIport)] #Open input MIDI ports

    print('Connected to physical MIDI in-port:', inports[1].name, 'and virtual MIDI out-port:', outport.name + '.')
    ...
    while True:
        for msg in mido.ports.multi_receive(inports):
            ...
            if msg.type == 'program_change' and msg.channel == inputChannel: # Accept only program_change on channel 0
                ...
                print('Sysex message \t@', local_time)
                outport.send(msg)
            else:
